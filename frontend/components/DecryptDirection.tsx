...
import { FUTURES_ADDRESS, POSITION_MANAGER_ADDRESS } from '@/lib/contracts';
...
export function DecryptDirection({ handle }: Props) {
  ...
  if (raw !== undefined) {
    const isLong = raw === 1n;
    return (
      <span className="badge" style={{ color: isLong ? 'var(--green)' : 'var(--red)' }}>
        {isLong ? 'Long ▲' : 'Short ▼'}
      </span>
    );
  }
